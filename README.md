...
启用 E2EE 后，在 Supabase SQL Editor 额外执行 [`supabase/migration_e2ee.sql`](supabase/migration_e2ee.sql)（为 `tasks` 表增加 `enc` 密文字段）。

## 端到端加密（隐私）

登录用户的任务内容在浏览器内加密后再同步；云端与项目运维**无法读取明文**。机制说明见 **[docs/PRIVACY-E2EE.md](docs/PRIVACY-E2EE.md)**（可转发给担心隐私的用户）。

要点：

- RSA 密钥对 + AES 数据密钥；私钥由**登录密码**加密保护
- 刷新页面后需再输入密码「解锁」才能读写加密任务
- 忘记密码则云端密文**不可恢复**
