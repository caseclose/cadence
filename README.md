...
<p align="center">自适应退避提醒 · 端到端加密 · 跨设备同步 · 开源自用</p>
...
<p align="center">挂起任务 · Markdown 备忘录 · 模板与统计 · Web Push / 飞书企微钉钉</p>
...
<p align="center">
  <img src="docs/screenshots/cadence-home.png" alt="Cadence 中文主界面：挂起任务、Markdown 备忘录与中英文切换" width="960" />
</p>

<p align="center"><sub>中文主界面 · 本地模式 · 任务挂起、Markdown 备忘录和语言切换</sub></p>

## 核心机制：自适应退避

每个挂起任务带一个预计时长 ETA，第一次提醒正好落在 ETA。到点后可完成、标记还没好、稍后处理、重估时长，或在已完成后重新挂起。快速延后会在原定提醒时间上累加，不会缩短已有等待时间。间隔按两种策略自适应：
...
- **挂起 + 自适应提醒**：完成、还没好、没空看、重估、重新挂起；提醒弹窗提供 `+10 分钟`、`+1 小时`、明天等快速延后
- **备忘录（Markdown）**：任务可附带命令、链接和检查清单；支持编辑/预览与代码块复制，到点提醒时一并展示
- **模板与快速录入**：把常用任务保存为模板；支持 URL 参数预填任务标题、ETA、策略和优先级
- **复盘统计**：记录完成、延后与重估事件，展示完成数、ETA 偏差中位数/P90 和平均提醒次数
- **后台提醒与摘要**：Web Push、飞书 / 企微 / 钉钉 Webhook，以及可选的每日通用摘要；Webhook 可单独选择是否发送任务明文（见 [docs/PUSH.md](docs/PUSH.md)）
- **可选云同步**：Supabase Auth + Realtime；任务、模板和统计事件均可端到端加密。同步通过单调版本、条件写入和删除墓碑避免延迟写入覆盖新状态
- **中英文与主题**：可切换中文 / English 以及浅色 / 深色 / 跟随系统主题；设置保存在当前浏览器，术语附带悬停说明

## 通知方式

Cadence 提供三种即时通知通道与一项可选的每日摘要，可按使用环境组合开启：
...
| **每日摘要** | 每日回顾待办 | 按设定时区和时间，通过已配置的 Webhook 发送任务数量摘要；不包含 E2EE 任务内容 |

界面中的「开启网页通知」和「开启后台推送」是两条独立通道。大陆用户建议优先配置群机器人 Webhook。每日摘要是独立设置，不替代到点提醒。
...
- 跨设备同步（可选）：Supabase（Auth + Postgres + Realtime）；任务同步使用版本决胜、条件写入与删除墓碑处理并发和乱序请求，未配置时退化为纯本地 localStorage
- 登录用户的任务内容、备忘录、模板与统计事件均在客户端 **端到端加密** 后再上云；云端仅保留推送所需的时间与状态元数据
...
当前版本优先支持 Android PWA，无需安装 Android Studio 或单独维护一套客户端：

1. 用 Android Chrome 打开 [在线版本](https://caseclose.github.io/cadence/)。
2. 登录后打开浏览器菜单，选择「安装应用」或「添加到主屏幕」。
3. 从桌面启动 Cadence，即可使用独立窗口和离线界面缓存。
4. 在应用内登录同一账号，任务会通过 Supabase 与 Web、CLI 等端同步。

PWA 离线缓存只负责打开界面；任务的云端同步仍需要网络。Android 后台提醒可使用 Web Push，具体可用性取决于浏览器和系统电池策略。后续如需 APK，再在此基础上接入 Capacitor 和 Android 本地通知。
