...
让登录用户在**关页 / 锁屏**时也能收到任务到点提醒。默认情况下服务器只知道「何时提醒」（`next_fire_at` 明文），不知任务内容（仍在 `enc` 密文）；推送 / 群消息均为**通用文案**。

如确实需要在群里直接看到任务内容，可在「提醒通道」中开启「发送任务明文内容」。这会将标题和备忘录明文同步到云端专用字段，并发送给已启用的机器人平台；该选项默认关闭，Web Push 仍不发送明文。
...
默认推送 / 群消息不含任务明文；若主动开启 Webhook 明文选项，标题和备忘录会发送给对应机器人平台。Webhook URL 仅本人可见（RLS）。见 [PRIVACY-E2EE.md](PRIVACY-E2EE.md)。
