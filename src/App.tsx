      ...
      <main className="main">
        ...
        {templates.length > 0 && (
          <section className="templates-section">
            ...
            <div className="template-grid">{templates.map((template) => <article className="template-card" key={template.id}><div><strong>{template.title}</strong><span><TermTip hintKey={template.strategy === 'converging' ? 'convergingHint' : template.strategy === 'exponential' ? 'exponentialHint' : 'recurringHint'}>{template.strategy === 'converging' ? t('convergingShort') : template.strategy === 'exponential' ? t('exponentialShort') : t('recurringShort')}</TermTip> · <TermTip hintKey={template.strategy === 'recurring' ? 'recurringHint' : 'etaHint'}>{template.strategy === 'recurring' ? t('recurringEvery') : 'ETA'}</TermTip> {formatDuration(template.etaMs)}</span></div><div className="task-actions"><button type="button" className="ghost" onClick={() => addFromTemplate(template.id)}>{t('useTemplate')}</button><button type="button" className="ghost danger" onClick={() => deleteTemplate(template.id)}>{t('delete')}</button></div></article>)}</div>
          </section>
        )}
